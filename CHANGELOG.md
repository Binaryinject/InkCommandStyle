...
- 🐛 修复保存文件时自动打开预览面板的问题
- 修复对话中自定义变量无法高亮的问题
- Live Update 只在预览面板已打开时生效

### 改进
- 🎨 自定义变量使用青色 (#4ec9b0)，与 @ 命令明显区分
- 优化语法高亮规则，支持嵌套模式匹配
...
### 修复
- 🔧 修复大纲中无法识别 `=xxxx` 格式（无空格）的 Stitch
- 支持 `= stitch` 和 `=stitch` 两种写法
- 修复跳转功能中对无空格 Stitch 的识别
- 改进正则表达式匹配，允许 `=` 后零个或多个空格
...
### 修复
- 🎯 修复故事预览面板点击跳转后光标不可见的问题
- 编辑器获得焦点以显示光标位置
- 光标定位在目标行行首，方便立即编辑
- 焦点保持在编辑器，提升编辑体验

### 改进
- 优化跳转交互流程，点击文本后可直接在编辑器中编辑
- 简化焦点管理逻辑，移除自动返回预览的延迟机制
